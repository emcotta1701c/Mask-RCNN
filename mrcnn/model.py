...
class ProposalLayer(KL.Layer):
    ...
    def __init__(self, proposal_count, config, **kwargs):
    ...
    def call(self, inputs):
        ...
        def nms(boxes, scores):
            indices = tf.image.non_max_suppression(
                ...
                self.config.RPN_NMS_THRESHOLD, name="rpn_non_max_suppression")
        ...
        def soft_nms(boxes, scores):
            indices, _ = tf.image.non_max_suppression_with_scores(
                boxes, scores, self.proposal_count,
                self.config.SOFT_NMS_THRESHOLD, self.config.SOFT_NMS_SIGMA, name="rpn_soft_non_max_suppression")
            proposals = tf.gather(boxes, indices)
            # Pad if needed
            padding = tf.maximum(self.proposal_count - tf.shape(proposals)[0], 0)
            proposals = tf.pad(proposals, [(0, padding), (0, 0)])
...
def fpn_graph(C2, C3, C4, C5, config):
    # Top-down Layers
...
#  Bi-FPN
############################################################
# Credit to: https://flash10042.medium.com/efficientdet-with-tensorflow-3c5f9df872a2
...
class BiFPN_Node(KL.Layer):
    def __init__(self, channels, kernel_size=3, depth_multiplier=1, epsilon=1e-4):
    ...
    def build(self, input_tensor):
        assert len(inputs.shape) == 5, "bi_fpn_node(): Error, input dimension incompatible: " + str(inputs.shape)
        # input_shape: #layers_contributing_to_input, batch_size, h, w, c_in
        num_inputs = len(input_tensor)
        # batch_size = len(input_tensor[0])
        ...
        self.conv2d = KL.SeparableConv2D(
            self.channels,
            self.kernel_size,
            padding='same',
        ...
        self.bn = BatchNorm()   # axis=-1 is default, which is correct for channels_last
        self.act = SiLU()
    
    def call(self, inputs, train_bn=False):
        assert len(inputs.shape) == 5, "bi_fpn_node(): Error, input dimension incompatible: " + str(inputs.shape)
        # input_shape: #layers_contributing_to_input, batch_size, h, w, c_in
        # weights shape: batch_size, c_out
        norm = tf.math.reduce_sum(self.w, axis=0) + self.epsilon
        ...
        bnorm = self.bn(conv, training=train_bn)
        return self.act(bnorm)

def bi_fpn_first_step(C2, C3, C4, C5, channels):
    # Top-down Layers (reduced version of FPN)
    # TODO: add assert to varify feature map sizes match what's in config
    """
    P4 = KL.Add()([
        KL.UpSampling2D(size=(2, 2))(C5),
        KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1))(C4)])

    P3 = KL.Add()([
        KL.UpSampling2D(size=(2, 2))(P4),
        KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1))(C3)])

    P2 = KL.Add()([
        KL.UpSampling2D(size=(2, 2))(P3),
        KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1))(C2)])

    # Attach 3x3 conv to all P layers to get the final feature maps.
    P2 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(P2)
    P3 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(P3)
    P4 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(P4)
    """

    P4 = BiFPN_Node(channels)([KL.UpSampling2D(size=(2, 2))(C5), C4])
    
    P3 = BiFPN_Node(channels)([KL.UpSampling2D(size=(2, 2))(P4), C3])
    
    P2 = BiFPN_Node(channels)([KL.UpSampling2D(size=(2, 2))(P3), C2])

    return [P2, P3, P4]

def bi_fpn_second_step(C1, C2, C3, C4, C5, P2, P3, P4, channels):
    Q1 = BiFPN_Node(channels)([KL.UpSampling2D(size=(2, 2))(P2), C1])
    Q2 = BiFPN_Node(channels)([KL.AveragePooling2D(size=(2, 2))(Q1), P2, C2])
    Q3 = BiFPN_Node(channels)([KL.AveragePooling2D(size=(2, 2))(Q2), P3, C3])
    Q4 = BiFPN_Node(channels)([KL.AveragePooling2D(size=(2, 2))(Q3), P4, C4])
    Q5 = BiFPN_Node(channels)([KL.AveragePooling2D(size=(2, 2))(Q4), C5])

    return [Q1, Q2, Q3, Q4, Q5]

class BiFPN_Layer(KL.Layer):
    def __init__(self, channels):
        super(BiFPN_Layer, self).__init__()
        self.channels = channels

    def call(C1, C2, C3, C4, C5):
        P2, P3, P4 = bi_fpn_first_step(C2, C3, C4, C5, self.channels)
        Q1, Q2, Q3, Q4, Q5 = bi_fpn_second_step(C1, C2, C3, C4, C5, P2, P3, P4, self.channels)

        return [Q1, Q2, Q3, Q4, Q5]

def bi_fpn_graph(C1, C2, C3, C4, C5, config):
    # TODO: add assert to varify feature map sizes match what's in config
    
    # C1, C2, C3, C4, C5 = BiFPN_Layer(config.TOP_DOWN_PYRAMID_SIZE)(C1, C2, C3, C4, C5)

    if "efficientnet" in config.BACKBONE:
        # EfficientNet + Bi-FPN = EfficientDet
        # get number of layers from research paper
        model_ind = config.BACKBONE[14] # i.e., efficientnet_b0, [14] = '0'
        nums_channels = [64, 88, 112, 160, 224, 288, 384, 384]
        nums_layers = [3, 4, 5, 6, 7, 7, 8, 8]
        num_channels = nums_channels[model_ind]
        num_layers = nums_layers[model_ind]

        if num_channels != config.TOP_DOWN_PYRAMID_SIZE:
            print("bi_fpn_graph() warning, pyramid size requested does not match model.")
            print("EfficientDet models have a predetermined Bi-FPN channel count.")
            print("Proceeding with automatically chosen pyramid size:", str(num_channels))
            print("Your chosen pyramid channel count was:", str(config.TOP_DOWN_PYRAMID_SIZE))
        
        if num_layers != config.BIFPN_NUM_LAYERS:
            print("bi_fpn_graph() warning, Bi-FPN pyramid layer count requested does not match model.")
            print("EfficientDet models have a predetermined Bi-FPN layer count.")
            print("Proceeding with automatically chosen pyramid layer count:", str(num_layers))
            print("Your chosen pyramid layer count was:", str(config.BIFPN_NUM_LAYERS))
        
        B1 = KL.Conv2D(num_channels, (3, 3), padding="SAME")(C1)
        B2 = KL.Conv2D(num_channels, (3, 3), padding="SAME")(C2)
        B3 = KL.Conv2D(num_channels, (3, 3), padding="SAME")(C3)
        B4 = KL.Conv2D(num_channels, (3, 3), padding="SAME")(C4)
        B5 = KL.Conv2D(num_channels, (3, 3), padding="SAME")(C5)

        for i in range(0, num_layers):
            B1, B2, B3, B4, B5 = BiFPN_Layer(num_channels)(B1, B2, B3, B4, B5)
        
        return [B1, B2, B3, B4, B5]

    else:   # custom methodology to deal with standard backbones that have fewer stages than EfficientNet
        # B6 is used for the 5th anchor scale in RPN. Generated by
        # subsampling from P5 with stride of 2.
        assert config.BIFPN_NUM_LAYERS >= 1, "bi_fpn_graph() error; config.BIFPN_NUM_LAYERS is invalid: " + config.BIFPN_NUM_LAYERS
        
        B1 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(C1)
        B2 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(C2)
        B3 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(C3)
        B4 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(C4)
        B5 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(C5)
        
        for i in range(0, config.BIFPN_NUM_LAYERS):
            B1, B2, B3, B4, B5 = BiFPN_Layer(num_channels)(B1, B2, B3, B4, B5)
        
        B6 = KL.MaxPooling2D(pool_size=(1, 1), strides=2)(B5)
...
# to address an error when running with a higher Keras version
class AnchorsLayer(KL.Layer):   # replacement for Keras Lambda layer with tf.Variable, which is not good practice
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Top-down Layers
        if config.USE_BIFPN:
            P2, P3, P4, P5, P6 = bi_fpn_graph(C1, C2, C3, C4, C5, config)
        else:
            P2, P3, P4, P5, P6 = fpn_graph(C2, C3, C4, C5, config)
        ...
        if mode == "training":
            ...
            anchors = AnchorsLayer(anchors, name="anchors")(input_image)    # For higher Keras version
        else:
