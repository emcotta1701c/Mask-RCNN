...
class bi_fpn_node(KL.Layer):
    def __init__(self, channels, kernel_size=3, depth_multiplier=1, epsilon=1e-4, name='bifpn_node'):
    ...
    def build(self, input_tensor):
        self.w = self.add_weight(
            shape=(len(inputs), self.channels), # len(inputs) == batch_size; hopefully matches tutorial
            initializer="ones",
            name='sum_weights',
            trainable=True
        )
        ...
        self.bn = BatchNorm()
        # self.act = SiLU()
        self.act = KL.Activation(tf.nn.silu)    # If doesn't work, try defining silu Layer: x = x * sigmoid(x)
...
def bi_fpn_topdown(C2, C3, C4, C5, size):
    ...
    P5 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME")(P5)
    # P6 is used for the 5th anchor scale in RPN. Generated by
    # subsampling from P5 with stride of 2.
...
def bi_fpn_bottomup(C2, C3, C4, C5, size):

def bi_fpn_layer(C2, C3, C4, C5, size):
    

    return [P2, P3, P4, P5, P6]

def bi_fpn_graph(C2, C3, C4, C5, config):
    # Top-down Layers
    # TODO: add assert to varify feature map sizes match what's in config

    P5 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1))(C5)
